...
# ── 8. COMBINED HR PER TREATMENT GROUP ───────────────────────
# Main effect = HR in CDK4/6i (reference)
# Combined = main effect × interaction term

get_est <- function(df, pattern) {
  val <- df %>% filter(grepl(pattern, term)) %>% pull(estimate)
  if (length(val) == 0 || is.na(val[1])) return(NA)
  val[1]
}

get_p <- function(df, pattern) {
  val <- df %>% filter(grepl(pattern, term)) %>% pull(p.value)
  if (length(val) == 0 || is.na(val[1])) return(NA)
  val[1]
}
...
combined_hr <- tibble(
  ...
  HR_combined = c(
    ...
    get_est(ccnd1_results,  "CCND1_cna$") * get_est(ccnd1_results, "EGFR"),
    get_est(ccnd1_results,  "CCND1_cna$") * get_est(ccnd1_results, "Immunotherapy"),
    get_est(cdkn2a_results, "CDKN2A_cna$"),
    get_est(cdkn2a_results, "CDKN2A_cna$") * get_est(cdkn2a_results, "Immunotherapy")
  ),
  Interpretation = c(
    "Harmful in CDK4/6i — 3x hazard",
    "Effect attenuated — near neutral",
    "Effect attenuated — not significant",
    "Protective in CDK4/6i — 77% lower hazard",
    "Reversed — harmful under immunotherapy"
  )
)
