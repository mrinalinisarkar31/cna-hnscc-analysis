...
# ── 3. CNA BINARY VARIABLES (NO MUTATION MIXING) ─────────────
master <- master %>%
  mutate(
...
# ── 8. SAFE COMBINED HR EXTRACTION ───────────────────────────
get_hr <- function(df, pattern) {
  df %>% filter(grepl(pattern, term)) %>% pull(estimate)
}

ccnd1_main <- get_hr(ccnd1_results, "CCND1_cna$")
ccnd1_egfr <- get_hr(ccnd1_results, "EGFR")
ccnd1_io   <- get_hr(ccnd1_results, "Immunotherapy")

cdkn2a_main <- get_hr(cdkn2a_results, "CDKN2A_cna$")
cdkn2a_io   <- get_hr(cdkn2a_results, "Immunotherapy")
...
combined_hr <- tibble(
  ...
  HR = c(
    ccnd1_main,
    ccnd1_main * ccnd1_egfr,
    ccnd1_main * ccnd1_io,
    cdkn2a_main,
    cdkn2a_main * cdkn2a_io
  )
)

print(combined_hr)

# ── 9. BH CORRECTION (SAFE INDEXING) ─────────────────────────
get_p <- function(df, pattern) {
  df %>% filter(grepl(pattern, term)) %>% pull(p.value)
}
...
# ── 11. SAVE OUTPUTS ─────────────────────────────────────────
write.csv(uni_results, "uni_results.csv", row.names = FALSE)
write.csv(cohort_overview, "cohort_overview.csv", row.names = FALSE)
write.csv(combined_hr, "combined_hr.csv", row.names = FALSE)
write.csv(all_p, "bh_results.csv", row.names = FALSE)
write.csv(cna_summary, "cna_summary.csv", row.names = FALSE)
